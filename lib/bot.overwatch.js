    ...
    this.thread = setInterval(function() {
        async.eachSeries(object.users, function(user, callback) {
            request({
                ...
                var divName = 'competitive-rank">';
                var rankDivName = 'h6">';
                var rankEndName = '</div>';

                var divIndex = body.indexOf(divName);
                var rankDivIndex = body.indexOf(rankDivName, divIndex);
                var rankEndIndex = body.indexOf(rankEndName, rankDivIndex);

                var currentRank = body.substr(rankDivIndex + rankDivName.length, rankEndIndex - (rankDivIndex + rankDivName.length));
...
Overwatch.prototype.players = function() {
    ...
    for (var i in this.ranks) {
        result += i.substr(0, i.indexOf('-')) + ': ' + this.ranks[i] + "\n";
    }
